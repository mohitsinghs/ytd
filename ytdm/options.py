...
def parseOpts(overrideArguments=None):
    def _readOptions(filename_bytes, default=[]):
        ...
        try:
            # FIXME: https://github.com/ytdl-org/ytdm/commit/dfe5fa49aed02cf36ba9f743b11b0903554b5e56
    ...
    def _readUserConf():
        ...
        if xdg_config_home:
            userConfFile = os.path.join(xdg_config_home, "ytdm", "config")
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(xdg_config_home, "ytdm.conf")
        else:
            userConfFile = os.path.join(
                compat_expanduser("~"), ".config", "ytdm", "config"
            )
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(
                    compat_expanduser("~"), ".config", "ytdm.conf"
                )
        ...
        if userConf is None:
            ...
            if appdata_dir:
                userConf = _readOptions(
                    os.path.join(appdata_dir, "ytdm", "config"), default=None
                )
                if userConf is None:
                    userConf = _readOptions(
                        os.path.join(appdata_dir, "ytdm", "config.txt"), default=None
                    )

        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser("~"), "ytdm.conf"), default=None
            )
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser("~"), "ytdm.conf.txt"), default=None
            )
    ...
    general.add_option(
        ...
        dest="default_search",
        metavar="PREFIX",
        help='Use this prefix for unqualified URLs. For example "gvsearch2:" downloads two videos from google videos for ytdm "large apple". Use the value "auto" to let ytdm guess ("auto_warning" to emit a warning when guessing). "error" just throws an error. The default value "fixup_error" repairs broken URLs, but emits an error if this is not possible instead of searching.',
    )
    general.add_option(
        "--ignore-config",
        action="store_true",
        help="Do not read configuration files. "
        "When given in the global configuration file /etc/ytdm.conf: "
        "Do not read the user configuration in ~/.config/ytdm/config "
        "(%APPDATA%/ytdm/config.txt on Windows)",
    )
    general.add_option(
        "--config-location",
    ...
    authentication.add_option(
        ...
        dest="password",
        metavar="PASSWORD",
        help="Account password. If this option is left out, ytdm will ask interactively.",
    )
    authentication.add_option(
        "-2",
        "--twofactor",
    ...
    adobe_pass.add_option(
        ...
        dest="ap_password",
        metavar="PASSWORD",
        help="Multiple-system operator account password. If this option is left out, ytdm will ask interactively.",
    )
    adobe_pass.add_option(
        "--ap-list-mso",
        action="store_true",
    ...
    verbosity.add_option(
        ...
        action="store_true",
        default=False,
        help="Contact the ytdm server for debugging",
    )
    verbosity.add_option(
        "--no-call-home",
        dest="call_home",
        action="store_false",
        default=False,
        help="Do NOT contact the ytdm server for debugging",
    )
    ...
    filesystem.add_option(
        ...
        dest="continue_dl",
        default=True,
        help="Force resume of partially downloaded files. By default, ytdm will resume downloads if possible.",
    )
    filesystem.add_option(
        "--no-continue",
        action="store_false",
    ...
    filesystem.add_option(
        ...
        default=None,
        metavar="DIR",
        help="Location in the filesystem where ytdm can store some downloaded information permanently. By default $XDG_CACHE_HOME/ytdm or ~/.cache/ytdm . At the moment, only YouTube player files (for videos with obfuscated signatures) are cached, but that may change.",
    )
    filesystem.add_option(
        "--no-cache-dir",
        action="store_const",
    ...
    if overrideArguments is not None:
        ...
        if "--config-location" in command_line_conf:
            ...
            if os.path.isdir(location):
                location = os.path.join(location, "ytdm.conf")
            ...
            system_conf = _readOptions("/etc/ytdm.conf")
